...
# "processed" holds the information from data to classify along with the following information: for each round in the
# cross validation technique, append a column stating the fold number in which a given image is,
# and the class it was assigned at this point.
...
# <from training data>  <from training date>   in what fold it is in round 1  how it was classified in round 1
# ...
#
# avg_precision <value>
# avg_recall   <value>
...
def cross_evaluate_knn(training_data, k, sim_id, f, data_to_classify):
    #
    rc = [[f'Round_{i}', f'Class_{i}'] for i in range(1, f + 1)]
    processed = [data_to_classify[0] + [y for tp in rc for y in tp]]
    # Have fun!
...
def main():
    ...
    result = cross_evaluate_knn(training_data, opts['k'], opts['sim_id'], opts['f'], data_to_classify)
